$(function(){
	Slide={
		...
		Slide.start=function(){
			...
			var BoxWidth=(Slide.time+1)*100+'%';	//计算存放图片的盒子宽度
			$('.Box').css('width',BoxWidth);		//设置存放图片的盒子宽度
			$('.Box').css('left','0');				//设置存放图片盒子的left，以便使用jquery对其进行检测与操作
			var CaseWidth2=100/(Slide.time+1)+'%';//计算轮播单位的宽度
		...
		Slide.next=function(){
			if(Slide.canSlide==true){					//判断是否可以执行轮播
				if(Slide.thisImg==Slide.time) {	//判断是否是最后一个轮播单位，如果是变为第一个轮播单位
					$('.Box').css('left','0');
					Slide.thisImg=0;
				}
		...
		Slide.run=function(){
			return setInterval(function(){
				if(Slide.canSlide==true){					//判断是否可以执行轮播
					if(Slide.thisImg==Slide.time-1) { //判断是否是最后一个轮播单位，如果是变为第一个轮播单位
						$('.Box').css('left','0');
						Slide.thisImg=0;
					}
					Slide.canSlide=false;					//停止时间间隔事件，防止下次轮播提前
